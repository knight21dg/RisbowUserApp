...
import 'package:hyper_local/utils/services/google_directions_service.dart';
...
class _DeliveryTrackingPageState extends State<DeliveryTrackingPage> {
  ...
  Future<Uint8List> buildPinLocationMarkerBytes(
    ...
    final String? safeImageUrl = resolveImageUrl(imageUrl);
    if (safeImageUrl != null) {
      try {
        final data = await NetworkAssetBundle(Uri.parse(safeImageUrl)).load(safeImageUrl);
  ...
  Future<void> _onTrackingLoaded(DeliveryTrackingLoaded state) async {
    ...
    if (lat != null && lng != null) {
      final snapped = await GoogleDirectionsService.snapToRoad(latitude: lat, longitude: lng);
      if (snapped != null) {
        _deliveryPartnerLocation = LatLng(snapped.latitude, snapped.longitude);
      } else {
    ...
    return FlutterMap(
      ...
      children: [
        TileLayer(
          ...
          userAgentPackageName: 'com.knight21.risbow',
        ),
  ...
  Widget _buildDeliveryPartnerSection({
    ...
    final String profileImage = (deliveryBoyProfile ?? '').trim();
    final bool hasProfileImage = profileImage.isNotEmpty;
    final bool isValidUrl = hasProfileImage && (profileImage.startsWith('http://') || profileImage.startsWith('https://'));
    final String? safeProfileImage = isValidUrl ? resolveImageUrl(profileImage) : null;
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(
              radius: 25,
              backgroundImage: (hasProfileImage && safeProfileImage != null) ? NetworkImage(safeProfileImage) : null,
              backgroundColor: Colors.grey[200],
              child: !(hasProfileImage && safeProfileImage != null)
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              _buildOrderDetailRow(AppLocalizations.of(context)!.orderPlaced, orderPlaced),
            ],
          ),
        )
      ],
