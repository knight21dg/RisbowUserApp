...
class _RateYourExpCommentsState extends State<RateYourExpComments> {
  ...
  final Map<int, List<XFile>> _itemVideos = {};
  int? _deletedOrderItemId;
  bool _lastActionWasAdd = false;
  ...
  Future<void> _pickVideoForItem(int orderItemId) async {
    const int maxVideos = 1;
    final currentVideos = _itemVideos[orderItemId] ?? [];
    ...
    final ImagePicker picker = ImagePicker();
    final XFile? selected = await picker.pickVideo(source: ImageSource.gallery);

    if (selected != null) {
      setState(() {
        _itemVideos[orderItemId] = [
          ...currentVideos,
          selected,
        ];
      });
  ...
  void _removeVideo(int orderItemId, int index) {
    setState(() {
      _itemVideos[orderItemId]?.removeAt(index);
      if (_itemVideos[orderItemId]?.isEmpty ?? true) {
        _itemVideos.remove(orderItemId);
      }
    });
  }
  ...
  void _submitSingle(int orderItemId, int productId, OrderItems item) {
    ...
    setState(() {
      ...
      _lastActionWasAdd = !isUpdate;
    });
    final images = _itemImages[orderItemId] ?? [];
    final videos = _itemVideos[orderItemId] ?? [];
        ...
        BlocListener<ProductFeedbackBloc, ProductFeedbackState>(
          listener: (context, state) {
            if (state is ProductFeedbackLoaded) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                final msg = wasDelete
                    ? AppLocalizations.of(context)!.feedbackDeletedSuccessfully
                    : _lastActionWasAdd
        ...
        body: _items.isEmpty
          ...
          itemBuilder: (context, index) {
            ...
            return shouldShowEditable
              ...
              productName: item.variant?.title ?? item.variantTitle ?? item.title ?? 'Unknown Product',
              orderItemId: orderItemId,
              productId: productId,
              item: item,
            )
...
// --- READ-ONLY CARD (Already Reviewed) ---
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if ((item.userReview?.reviewImages != null && item.userReview!.reviewImages!.isNotEmpty) || (item.userReview?.reviewVideos != null && item.userReview!.reviewVideos!.isNotEmpty)) ...[
            if (titleText.isNotEmpty || commentText.isNotEmpty)
              SizedBox(height: 8.h),
            // Review images & videos
            _buildReviewImagesDisplay(item.userReview?.reviewImages ?? []),
            _buildReviewVideosDisplay(item.userReview?.reviewVideos ?? []),
          ],
          SizedBox(height: 8.h),
          Align(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 12.h),
          item.userReview?.id != null
              ? _buildApiVideosSection(item.userReview?.reviewVideos ?? [])
              : _videosSectionForItem(orderItemId, _itemVideos[orderItemId] ?? []),
          SizedBox(height: 12.h),
          SizedBox(
            width: double.infinity,
            child: CustomButton(
  ...
  Widget _videosSectionForItem(int orderItemId, List<XFile> videos) {
    const int maxVideos = 1;
    const String allowedExtensions = 'MP4, MOV';
    const String maxSizePerVideo = '50 MB';

    final int selectedCount = videos.length;
    final bool canAddMore = selectedCount < maxVideos;
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text(
              '$selectedCount / $maxVideos',
              style: TextStyle(
                fontSize: 11.sp,
                color: selectedCount == maxVideos
                    ? AppTheme.errorColor
                    : Colors.grey[700],
                fontWeight: FontWeight.w500,
              ),
        ...
        DashedContainer(
          ...
          child: videos.isEmpty
              ? _buildEmptyVideoStateForItem(orderItemId)
              : _buildVideoGridWithAddButtonForItem(orderItemId, videos, canAddMore),
        ),
        Padding(
          padding: EdgeInsets.only(top: 6.h, left: 2.w),
          child: Text(
            '• Max $maxVideos video • $allowedExtensions • $maxSizePerVideo per video',
            style: TextStyle(
              fontSize: 10.sp,
              color: Colors.grey[600],
              height: 1.3,
  ...
  Widget _buildEmptyVideoStateForItem(int orderItemId) {
    return InkWell(
      onTap: () => _pickVideoForItem(orderItemId),
  ...
  Widget _buildVideoGridWithAddButtonForItem(int orderItemId, List<XFile> videos, bool canAddMore) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Wrap(
              ...
              Stack(
                children: [
                  Container(
                    ...
                    child: Center(
                      child: Icon(Icons.videocam, color: Colors.black54, size: 30),
                    ),
                  ),
                  Positioned(
                    right: 0,
                    top: 0,
                    child: InkWell(
                      onTap: () => _removeVideo(orderItemId, i),
                      child: Container(
                        decoration: BoxDecoration(
        ...
        Wrap(
          ...
          children: [
            for (int i = 0; i < apiImages.length; i++)
              ClipRRect(
                borderRadius: BorderRadius.circular(10.r),
                child: (apiImages[i].trim().isNotEmpty)
                    ? CustomImageContainer(
                        imagePath: apiImages[i],
                        width: 76.w,
                        height: 76.w,
                        fit: BoxFit.cover,
    ...
    return Wrap(
      ...
      children: [
        for (int i = 0; i < reviewImages.length; i++)
          ClipRRect(
            borderRadius: BorderRadius.circular(10.r),
            child: reviewImages[i].trim().isNotEmpty
                ? CustomImageContainer(
                    imagePath: reviewImages[i],
                    width: 76.w,
                    height: 76.w,
                    fit: BoxFit.cover,
                    fallbackAsset: 'assets/images/placeholder.png',
  ...
  Widget _buildApiVideosSection(List<ReviewVideo> apiVideos) {
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 6.h),
        _buildReviewVideosDisplay(apiVideos),
      ],
    );
  }

  Widget _buildReviewVideosDisplay(List<ReviewVideo> reviewVideos) {
    ...
    return Wrap(
      ...
      runSpacing: 10.h,
      children: [
        for (int i = 0; i < reviewVideos.length; i++)
          if (reviewVideos[i].url != null)
            ClipRRect(
              borderRadius: BorderRadius.circular(10.r),
              child: SizedBox(
                width: 76.w,
                height: 76.w,
                child: ProductVideoPlayer(videoUrl: reviewVideos[i].url ?? '', isActive: true),
              ),
            ),
      ],
    );
