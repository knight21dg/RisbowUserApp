...
class _CategoryListViewState extends State<_CategoryListView> {
  ...
  // Track if it's the initial load (to avoid auto-select)
  bool _isInitialLoad = true;

  // ─── Level 2: Product view state ───
  bool _isShowingProducts = false;
  SubCategoryData? _selectedSubcategory;
  int _selectedSubcategoryIndex = -1; // -1 means no selection initially
  ...
  /// When a subcategory is tapped: navigate to stores page with parent context
  void _onSubcategoryTap(SubCategoryData subcategory, int index) {
    final blocState = context.read<AllCategoriesBloc>().state;
    if (blocState is AllCategoriesLoaded) {
      final rootCategories = blocState.subCategoryData.where((c) => c.parentId == null || c.parentId == 0).toList();
      final parentCategories = rootCategories.isNotEmpty ? rootCategories : blocState.subCategoryData;
      if (_selectedCategoryIndex < parentCategories.length) {
        final parent = parentCategories[_selectedCategoryIndex];
        GoRouter.of(context).push(
          AppRoutes.categoryStores,
          extra: {
...
listener: (context, state) {
            if (state is AllCategoriesLoaded && state.subCategoryData.isNotEmpty && !state.isLoadingMore) {
              ...
              if (_subcategories.isEmpty && !_isShowingProducts && !_isLoadingSubcategories && !_isInitialLoad) {
  ...
  Widget _buildSubcategoryGridSkeleton() {
    return GridView.builder(
      ...
      itemBuilder: (context, index) => Column(
        children: [
          ShimmerWidget.circular(width: 60.w, height: 60.w, isBorder: false, borderRadius: 12),
          SizedBox(height: 8.h),
          ShimmerWidget.rectangular(width: 50.w, height: 10, isBorder: false, borderRadius: 4),
        ],
      ),
    );
  }
  ...
  /// Skeleton loader for right side (subcategories list)
  Widget _buildSubcategoryListSkeleton() {
    return Padding(
        ...
        itemBuilder: (context, index) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ShimmerWidget.rectangular(width: double.infinity, height: 10, isBorder: false, borderRadius: 4),
              SizedBox(height: 6.h),
              ShimmerWidget.rectangular(width: 40.w, height: 10, isBorder: false, borderRadius: 4),
            ],
          ),
        ),
      ),
  ...
  Widget _buildSubcategoryPanel() {
    if (_isLoadingSubcategories) {
      return _buildSubcategoryListSkeleton();
    }
    ...
    if (filteredSubcategories.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Image.asset(
              'assets/images/empty-states/no-search-results.png',
              width: 120.w,
              height: 120.w,
            ),
            SizedBox(height: 12.h),
            ...
            Expanded(
              child: Container(
                ...
                child: (subcat.image ?? '').trim().isNotEmpty
                    ...
                    : Center(
                        child: Text(
                          (subcat.title ?? '').isNotEmpty
                              ? subcat.title![0].toUpperCase()
                              : '',
                          style: TextStyle(
                            fontSize: 24.sp,
                            fontWeight: FontWeight.bold,
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: (subcat.image ?? '').trim().isNotEmpty
                          ...
                          : Center(
                              child: Text(
                                (subcat.title ?? '').isNotEmpty ? subcat.title![0].toUpperCase() : '',
                                style: TextStyle(
                                  fontSize: 24.sp,
                                  fontWeight: FontWeight.bold,
