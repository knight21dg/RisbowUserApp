...
class BillSummaryWidget extends StatelessWidget {
  ...
  final double? coinsDiscountRupee;
        ...
        child: Column(
          ...
          children: [
            ...
            _buildItemRow(
              ...
              hasDiscount: itemsOriginalPrice > itemsDiscountedPrice,
            ),
            ...
            // Platform charge row
            if (platformCharge != null && platformCharge! > 0)
              _buildItemRow(
                context: context,
                icon: Icons.miscellaneous_services,
                label: 'Platform Charge',
                finalPrice: '$currency${platformCharge!.toStringAsFixed(2)}',
              ),
            if (platformCharge != null && platformCharge! > 0) const SizedBox(height: 16),

            // CGST row
            if ((cgst != null && cgst! > 0) || (sgst != null && sgst! > 0) || GstUtils.hasGst)
              if (itemsDiscountedPrice > 0)
                _buildItemRow(
                  context: context,
                  icon: Icons.account_balance,
                  label: 'CGST (${GstUtils.getCgstPercent().toStringAsFixed(0)}%)',
                  finalPrice: '$currency${(cgst ?? GstUtils.getCgstAmountFromExclusive(itemsDiscountedPrice)).toStringAsFixed(2)}',
                ),
            if ((cgst != null && cgst! > 0) || (sgst != null && sgst! > 0) || GstUtils.hasGst)
              if (itemsDiscountedPrice > 0) const SizedBox(height: 16),
            ...
            if ((cgst != null && cgst! > 0) || (sgst != null && sgst! > 0) || GstUtils.hasGst)
              if (itemsDiscountedPrice > 0)
                _buildItemRow(
                  context: context,
                  icon: Icons.account_balance,
                  label: 'SGST (${GstUtils.getSgstPercent().toStringAsFixed(0)}%)',
                  finalPrice: '$currency${(sgst ?? GstUtils.getSgstAmountFromExclusive(itemsDiscountedPrice)).toStringAsFixed(2)}',
                ),
            if ((cgst != null && cgst! > 0) || (sgst != null && sgst! > 0) || GstUtils.hasGst)
              if (itemsDiscountedPrice > 0) const SizedBox(height: 16),
            ...
            if(isFromOrderDetail == false) ...[
              ...
              if (promoCode != null && promoCode!.isNotEmpty) ...[
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _getPromoBackgroundColor(promoMode),
            ...
            // Promo error message (if present)
              ...
              // Coins redemption discount (if present)
              if (coinsDiscountRupee != null && coinsDiscountRupee! > 0) ...[
                _buildItemRow(
                  context: context,
                  icon: Icons.monetization_on,
                  label: 'Coins Discount',
                  additionalInfo: coinsToUse != null ? '$coinsToUse coins' : null,
                  finalPrice: '-$currency${coinsDiscountRupee!.toStringAsFixed(2)}',
                ),
                const SizedBox(height: 12),
              ],
            ],
  ...
  Color _getPromoBackgroundColor(String? promoMode) {
