...
class SimilarProductBloc extends Bloc<SimilarProductEvent, SimilarProductState> {
  SimilarProductBloc() : super(SimilarProductInitial()) {
    ...
    on<FetchRecommendedSimilarProduct>(_onFetchRecommendedSimilarProduct);
  }
  ...
  Future<void> _onFetchRecommendedSimilarProduct(FetchRecommendedSimilarProduct event, Emitter<SimilarProductState> emit) async {
    emit(SimilarProductLoading());
    try {
      final response = await repository.fetchRecommendedSimilarProduct(
        productId: event.productId,
        limit: event.limit,
      );

      if (response['success'] == true) {
        final List<ProductData> categoryProduct = List<ProductData>.from(
          (response['data'] as List).map((data) {
            // Handle response structure from recommendation API
            if (data is Map) {
        ...
        emit(SimilarProductLoaded(
          message: response['message'] ?? 'Similar products loaded',
          similarProduct: categoryProduct,
          hasReachedMax: true,
        ));
      } else {
        emit(SimilarProductFailure(error: response['message'] ?? 'Failed to load similar products'));
      }
    } catch (e) {
      emit(SimilarProductFailure(error: e.toString()));
    }
