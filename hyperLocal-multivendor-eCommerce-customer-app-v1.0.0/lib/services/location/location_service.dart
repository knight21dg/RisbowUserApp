...
class LocationService {
  ...
  static Future<UserLocation?> requestAndStoreLocationWithRetry() async {
    ...
    try {
      ...
      final zoneInfo = await getZoneFromApi(position.latitude, position.longitude);
      String? zoneId;
      String? zoneName;
  ...
  static Future<UserLocation?> storeLocationFromCoordinates(
    ...
    try {
      ...
      final zoneInfo = await getZoneFromApi(lat, lng);
      String? zoneId;
      String? zoneName;
      if (zoneInfo != null && zoneInfo['isDeliverable'] == true) {
        zoneId = zoneInfo['zoneId'];
  ...
  static Future<void> storeLocation(UserLocation location) async {
    _checkedLocations.remove('${location.latitude},${location.longitude}');
  ...
  static final Set<String> _checkedLocations = {};
  ...
  static Map<String, double> getCoordinates() {
    ...
    if (stored != null && stored.latitude > 0 && stored.longitude > 0) {
      ...
      // Trigger background zone check only if we haven't checked this location yet
      final locKey = '${stored.latitude},${stored.longitude}';
      if (!_checkedLocations.contains(locKey)) {
        _checkedLocations.add(locKey);
