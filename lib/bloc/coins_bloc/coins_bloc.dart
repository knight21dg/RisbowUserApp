...
class CoinsBloc extends Bloc<CoinsEvent, CoinsState> {
  ...
  CoinsSettingsModel? _cachedSettings;
  ...
  Future<void> _onFetchSettings(
    ...
    if (_cachedSettings != null) {
      emit(CoinsSettingsLoaded(_cachedSettings!));
      return;
    }
    emit(CoinsLoading());
    try {
      ...
      if (data.isNotEmpty) {
        _cachedSettings = CoinsSettingsModel.fromJson(data);
        emit(CoinsSettingsLoaded(_cachedSettings!));
      } else {
        emit(CoinsError('Failed to load coins settings'));
      }
    } catch (e) {
      emit(CoinsError(e.toString()));
    }
  ...
  /// Clear cached settings (call on logout / settings update)
  void clearCache() {
  ...
  Future<void> _onCalculateRedemption(
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        emit(CoinsError(response['message'] ?? 'Failed to calculate redemption'));
      }
    } catch (e) {
      emit(CoinsError(e.toString()));
    }
  ...
  void onError(error, stackTrace) {
    debugPrint('CoinsBloc error: $error');
