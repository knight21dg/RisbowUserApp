...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Future<void> _onAddToCart(AddToCart event, Emitter<CartState> emit) async {
    ...
    try {
      final productId = int.tryParse(event.item.productId) ?? 0;
      ...
      if (res['success'] == true && res['data'] != null) {
        ...
        if (itemsList != null) {
          final addedServerItem = itemsList.firstWhere(
            (serverItem) {
              final matchVariant = serverItem['product_variant_id'].toString() == event.item.variantId;
              final matchNoVariant = event.item.variantId == '0' &&
                  serverItem['product_id'].toString() == event.item.productId;
              return (matchVariant || matchNoVariant) &&
        ...
        switch (item.syncAction) {
          case CartSyncAction.add:
            ...
            if (res['success'] == true && res['data'] != null) {
              ...
              bool matchServerItem(Map serverItem, String variantId, String storeId, String productId) {
                final matchVariant = serverItem['product_variant_id'].toString() == variantId;
                final matchNoVariant = variantId == '0' &&
                    serverItem['product_id'].toString() == productId;
                return (matchVariant || matchNoVariant) &&
                    serverItem['store_id'].toString() == storeId;
              }
              ...
              if (serverItems != null) {
                for (final serverItem in serverItems) {
                  if (serverItem is Map &&
                      matchServerItem(serverItem, item.variantId, item.vendorId, item.productId)) {
