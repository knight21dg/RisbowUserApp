...
class _FlashSalesPageState extends State<FlashSalesPage> {
  ...
  Future<void> _loadFallbackProducts() async {
    try {
      final response2 = await http.get(
        Uri.parse('${ApiRoutes.searchApi}?limit=30&page=1'),
        headers: {'Content-Type': 'application/json'},
      ).timeout(const Duration(seconds: 15));
      ...
      if (response2.statusCode == 200) {
        ...
        if (data2['success'] == true && data2['data'] != null) {
          setState(() {
            _products = _transformProducts(data2['data']);
            _isLoading = false;
          });
        } else {
          setState(() {
            _hasError = true;
            _errorMessage = data2['message'] ?? 'Failed to load products';
            _isLoading = false;
          });
        }
      } else {
        setState(() {
          _hasError = true;
          _errorMessage = 'Server error: ${response2.statusCode}';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Widget _buildFlashDealCard(Map<String, dynamic> deal) {
    ...
    return GestureDetector(
      onTap: () => context.push('/product-detail/${deal['id']}'),
      child: Container(
        width: 160.w,
  ...
  Widget _buildImage(dynamic url, double height) {
    if (url?.toString().isNotEmpty == true) {
      return CachedNetworkImage(
        ...
        fit: BoxFit.cover,
        placeholder: (context, url) => Container(height: height, color: Colors.grey[200], child: Center(child: CircularProgressIndicator())),
  ...
  Widget _buildPrice(dynamic price, dynamic originalPrice) {
    ...
    return Row(
      children: [
        Text('\$${p.toStringAsFixed(2)}', style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.bold, color: AppTheme.primaryColor)),
        if (op > p) ...[
          SizedBox(width: 4.w),
          Text('\$${op.toStringAsFixed(2)}', style: TextStyle(fontSize: 10.sp, decoration: TextDecoration.lineThrough, color: Colors.grey)),
        ],
      ],
    );
  }
