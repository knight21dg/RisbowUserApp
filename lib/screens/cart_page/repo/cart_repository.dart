...
class CartRepository {
  ...
  Future<dynamic> addItemToCart ({
    required int productVariantId, required int storeId, required int quantity, int? productId
  }) async {
    try{
  ...
  Future<Map<String, dynamic>> updateItemQuantity ({
    ...
    try{
      ...
      throw ApiException('Failed to update item quantity');
    }
  }
  ...
  Future<List<GetCartModel>> getCartItems({
    ...
    try{
      ...
      // Send coin usage flag to backend. Keep parameter value coming from the
      // existing `useWallet` flag for now (we'll migrate callers later).
      if (useWallet != null) queryParams['use_coins'] = useWallet.toString();
