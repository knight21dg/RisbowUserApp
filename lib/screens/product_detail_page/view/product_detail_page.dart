...
class _ProductDetailPageState extends State<ProductDetailPage>
  ...
  void initState() {
    ...
    context.read<CoinsBloc>().add(FetchCoinsSettings());
  }
      ...
      child: CustomScaffold(
        ...
        bottomNavigationBar: BlocBuilder<ProductDetailBloc, ProductDetailState>(
          builder: (context, state) {
            ...
            final inclusivePrice = activeVariant.price > 0
                ? activeVariant.price.toDouble()
                : product.price;
            final inclusiveSpecialPrice = (activeVariant.specialPrice > 0
                ? activeVariant.specialPrice.toDouble()
                : product.specialPrice);
            final displayPrice = activeVariant.priceExcludeTax ?? GstUtils.getBasePrice(inclusivePrice);
            final displaySpecialPrice = activeVariant.specialPriceExcludeTax ?? GstUtils.getBasePrice(inclusiveSpecialPrice);
  ...
  Widget _buildStandardFooter(
    ...
    final l10n = AppLocalizations.of(context)!;
    final bool inStock = product.variants.isEmpty
        ? product.stock > 0
          ...
          Flexible(
            child: Column(
              ...
              children: [
                ...
                _buildCoinsBadge(displaySpecialPrice > 0 ? displaySpecialPrice : displayPrice),
              ],
            ),
          ),
          SizedBox(width: 8),
            ...
            Expanded(
              child: Align(
                ...
                child: ConstrainedBox(
                  constraints: BoxConstraints(maxWidth: 135, minWidth: 80),
                  child: BlocBuilder<CartBloc, CartState>(
                    builder: (context, cartState) {
                      final int productId = product.id;
                      int productVariantId = activeVariant.id;
                      final int storeId = product.variants.isEmpty
                          ? product.storeId
        ...
        children: [
          GestureDetector(
            ...
            child: Padding(
              ...
              child: Icon(TablerIcons.minus, size: 18.r, color: Colors.white),
            ),
          ),
          Text(
          ...
          GestureDetector(
            ...
            child: Padding(
              padding: EdgeInsets.symmetric(horizontal: 6.w),
              child: Icon(TablerIcons.plus, size: 18.r, color: Colors.white),
            ),
          ),
        ],
      ),
  ...
  Widget _buildCoinsBadge(double effectivePrice) {
    return BlocBuilder<CoinsBloc, CoinsState>(
      builder: (context, coinsState) {
        int coins = 0;
        if (coinsState is CoinsSettingsLoaded) {
          final percent = coinsState.settings.coinsOrderEarnPercent;
          if (percent > 0 && effectivePrice > 0) {
            coins = (effectivePrice * percent / 100).round();
          }
        }
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 4.w),
              Text(
                'Earn $coins coin${coins > 1 ? 's' : ''}',
                style: TextStyle(fontSize: 11.sp, color: Colors.amber.shade800, fontWeight: FontWeight.w600),
              ),
            ],
          ),
        );
  ...
  void _addToCart(ProductData product, ProductVariants activeVariant) {
    ...
    final int effectiveStock = product.variants.isEmpty
        ? product.stock
    ...
    final bool noVariants = product.variants.isEmpty;
    final double itemPrice = noVariants
        ? product.specialPrice > 0 ? product.specialPrice : product.price
        : selectedVariant.specialPrice.toDouble();
    final double itemOriginalPrice = noVariants
        ? product.price
        : selectedVariant.price.toDouble();
    final int effectiveStoreId = noVariants
        ? product.storeId
    ...
    final item = UserCart(
      ...
      vendorId: effectiveStoreId.toString(),
      name: product.title,
