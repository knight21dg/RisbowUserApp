...
class _HomePageState extends State<HomePage>
  ...
  late final ValueListenable<Box<dynamic>> _locationListenable;
  ...
  void _onLocationChanged() {
    ...
    final locationIdentifier = storedLocation == null
        ? null
        : '${storedLocation.latitude}_${storedLocation.longitude}_${storedLocation.fullAddress}_${storedLocation.area}_${storedLocation.city}_${storedLocation.pincode}_${storedLocation.zoneId}';
  ...
  void initState() {
    ...
    if (storedLocation == null || storedLocation.zoneId == null || storedLocation.zoneId!.isEmpty) {
    ...
    final box = Hive.box<dynamic>('userLocationBox');
    final rawLocation = box.get('user_location');
    _lastLocationIdentifier = rawLocation == null
        ? null
        : '${rawLocation.latitude}_${rawLocation.longitude}_${rawLocation.fullAddress}_${rawLocation.area}_${rawLocation.city}_${rawLocation.pincode}_${rawLocation.zoneId}';
    _locationListenable = box.listenable();
    _locationListenable.addListener(_onLocationChanged);
  ...
  Future<void> _checkLocationAndLoad() async {
    if (!LocationService.hasStoredLocation()) {
      ...
      if (stored != null && (stored.zoneId == null || stored.zoneId!.isEmpty)) {
        ...
        final zoneInfo = await LocationService.getZoneFromApi(
          stored.latitude,
          stored.longitude,
        );
  ...
  void _ensureValidTabIndex() {
    log(
      'Ensure Valid Tab Index ${(!mounted || !_canUseTabController || _isRedirecting)}',
    );
  ...
  void apiCalls(String slug) {
    ...
    try {
      if (_tabController.index == 0) {
        ...
        context.read<HomepageSectionBloc>().add(
          FetchHomepageSections(categorySlug: slug),
        );
        context.read<GetUserCartBloc>().add(FetchUserCart());
        context.read<GetAddressListBloc>().add(FetchUserAddressList());
      }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      if (DateTime.now().difference(_lastResumeTime).inSeconds < 10) return;
  ...
  void dispose() {
    ...
    _locationListenable.removeListener(_onLocationChanged);
  ...
  void _applyHomeGeneralSettingsToAppBar() {
    ...
    final String image = settings.backgroundImage.isNotEmpty
        ? settings.backgroundImage
        : '';
    final String? bgColor = settings.backgroundColor.isNotEmpty
      ...
      child: Column(
        ...
        children: [
          SizedBox(
            ...
            height: 56,
            child: Image.asset(
              isDark ? 'assets/images/app_logos/app-logo-dark.png' : 'assets/images/app_logos/app-logo-light.png',
              fit: BoxFit.contain,
              errorBuilder: (_, __, ___) => Icon(HeroiconsOutline.squares2x2, size: 28),
            ),
          ),
  ...
  Widget build(BuildContext context) {
    super.build(
      context,
    ); // Must call super.build() but ignore return value for AutomaticKeepAliveClientMixin
                ...
                List<Widget> tabBarViewChildren = [
                  CustomRefreshIndicator(
                    onRefresh: () async {
                      ...
                      context.read<CustomSalePageBloc>().add(
                        FetchCustomSalePages(),
                      );
                                  ...
                                  builder: (context, brandsState) {
                                    ...
                                    if (hasFailed) {
                                      return EmptyStatePage(
                                        ...
                                        imageAsset:
                                            'assets/images/icons/server_error.png',
                                        onRetry: () {
                                          setState(() {
                                            isRetry = true;
                                          });
                                                    ...
                                                    return Column(
                                                      ...
                                                      children: [
                                                        ...
                                                        ...state.pages
                                                            ...
                                                            .expand(
                                                              (page) => [
                                                                CustomSalePageListItemWidget(
                                                                  page: page,
                                                                  onTap: () =>
                                                                      context.push(
                                                                        '/custom-sale-page/${page.slug}',
                                                                      ),
                                                                ),
                                                                SizedBox(
                                                                  height: 16.h,
                  ...
                  _categories.asMap().entries.map((entry) {
                    ...
                    return CustomRefreshIndicator(
                      onRefresh: () async {
                        ...
                        context.read<HomepageSectionBloc>().add(
                          FetchHomepageSections(categorySlug: category.slug),
                        );
                      },
                      child: BlocBuilder<BannerBloc, BannerState>(
                        builder: (context, bannerState) {
                ...
                return NestedScrollView(
                  ...
                  body: () {
                    ...
                    if (storedLocation != null &&
                        (storedLocation.zoneId == null ||
                            storedLocation.zoneId!.isEmpty)) {
  ...
  void _loadMoreForCurrentTab(int tabIndex) {
    ...
    if (featureSectionState is FeatureSectionProductLoaded &&
      ...
      context.read<FeatureSectionProductBloc>().add(
        FetchMoreFeatureSectionProducts(slug: slug),
      );
