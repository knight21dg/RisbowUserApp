...
class DynamicHomepageSectionWidget extends StatelessWidget {
  ...
  Widget _buildDealOfDayCard(BuildContext context, HomepageProduct product, int index) {
    final gradients = [
      [const Color(0xFFFF6B35), const Color(0xFFFFAB47)],
  ...
  bool _hasDiscount(HomepageProduct product) {
    final mrp = product.mrp ?? 0;
    final price = product.price;
    return mrp > 0 && price > 0 && mrp > price;
  }

  String _getDiscountPercent(HomepageProduct product) {
    final mrp = product.mrp ?? 0;
    final price = product.price;
    if (mrp > 0 && price > 0) {
      return (((mrp - price) / mrp) * 100).toStringAsFixed(0);
    }
    return '0';
  }
