    ...
    try{
      ...
      if(response.first.success!){
        if(response.first.data!.isDeliverable!) {
          emit(CheckDeliveryZoneSuccess(
            message: response.first.message ?? 'Delivery is not available to this location',
            zoneId: response.first.data?.zoneId?.toString(),
            zoneName: response.first.data?.zone,
          ));
        } else {
          emit(CheckDeliveryZoneFailure(error: response.first.message ?? 'Delivery is not available to this location'));
        }
      } else {
        emit(CheckDeliveryZoneFailure(error: response.first.message ?? 'Failed to check delivery availability'));
      }
    }catch(e){
