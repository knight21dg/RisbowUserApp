...
class GetAddressListBloc extends Bloc<GetAddressListEvent, GetAddressListState> {
  ...
  Future<void> _onAddAddressRequest(AddAddressRequest event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListInitial) {
      await _onFetchUserAddressList(FetchUserAddressList(deliveryZoneId: event.deliveryZoneId), emit);
    }

    if (state is! GetAddressListLoaded) return;
  ...
  Future<void> _onUpdateAddressRequest(UpdateAddressRequest event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListInitial) {
      await _onFetchUserAddressList(FetchUserAddressList(), emit);
    }
    ...
    final currentState = state as GetAddressListLoaded;
      ...
      try {
        ...
        /*if (response['success'] == true) {
          final updatedAddressList = currentState.addressList.map((address) {
            if (address.id == event.addressId) {
              return AddressListData(
                ...
                updatedAt: response['data']?['updated_at'] ?? address.updatedAt,
              );
            }
            return address;
          }).toList();
          ...
          emit(GetAddressListFailed(error: response['message'] ?? 'Failed to update address'));
        }*/
      } catch (e) {
        emit(currentState.copyWith(isUpdating: false));
      }
