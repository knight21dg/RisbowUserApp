...
class UserWishlistBloc extends Bloc<UserWishlistEvent, UserWishlistState> {
  ...
  int? getWishlistItemId(int productId, int productVariantId, int storeId) {
    final cacheKey = _getCacheKey(productId, productVariantId, storeId);
    if (_localWishlistCache.containsKey(cacheKey)) {
      final cachedId = _localWishlistCache[cacheKey];
      // Return null if cache has invalid ID (-1 means pending or not properly saved)
  ...
  Future<void> _onGetUserWishlistRequest(GetUserWishlistRequest event, Emitter<UserWishlistState> emit) async {
    ...
    try{
      ...
      if (responseData == null) {
        emit(UserWishlistLoaded(message: 'No data', wishlistData: [], hasReachedMax: true));
        return;
      }
      
      final dataList = responseData['data'];
      if (dataList == null) {
        emit(UserWishlistLoaded(message: 'No wishlist', wishlistData: [], hasReachedMax: true));
        return;
      }
      
      wishlistData = List<WishlistData>.from(
        (dataList as List).map((data) => WishlistData.fromJson(data))
      );

      final currentTotal = int.tryParse(responseData['current_page']?.toString() ?? '1') ?? 1;
      final lastPageNum = int.tryParse(responseData['last_page']?.toString() ?? '1') ?? 1;
      ...
      if(response['success'] == true){
        ...
        emit(UserWishlistFailed(message: response['message'] ?? 'Failed'));
      } else {
        emit(UserWishlistFailed(message: response['message'] ?? 'Unknown error'));
      }
    }catch(e) {
  ...
  Future<void> _onGetMoreUserWishlistRequest(GetMoreUserWishlistRequest event, Emitter<UserWishlistState> emit) async {
    ...
    if(currentState is UserWishlistLoaded) {
      ...
      try{
        ...
        newWishlistData = List<WishlistData>.from(
          (dataList as List).map((data) => WishlistData.fromJson(data))
        );
