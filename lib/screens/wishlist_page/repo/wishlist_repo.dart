  ...
  Future<Map<String, dynamic>> getUserWishlist({
    ...
    try{
      ...
      if(response.statusCode == 200 && response.data != null){
        ...
        // Try alternate route
        final altResponse = await AppConstant.apiBaseHelper.getAPICall(
            '${ApiRoutes.getWishlistApiAlt}?page=$currentPage&per_page=$perPage',
            {}
        );
        if(altResponse.statusCode == 200 && altResponse.data != null){
          return altResponse.data;
        }
        return {'success': false, 'message': 'No response', 'data': {'data': []}};
      }
    }catch(e) {
  ...
  Future<Map<String, dynamic>> addItemInWishlist({
    ...
    try{
      ...
      if(response.statusCode == 200){
        ...
        // Try alternate route
        final altResponse = await AppConstant.apiBaseHelper.postAPICall(
          ApiRoutes.addItemInWishlistApiAlt,
          {
            'wishlist_title': wishlistTitle,
            'product_id': productId,
            'product_variant_id': productVariantId,
