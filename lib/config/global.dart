...
class Global {
  ...
  static Future<void> initialize() async {
    ...
    Box<UserDataModel> box = await Hive.openBox<UserDataModel>(_boxName);
    _userData = box.get(_userKey);
    debugPrint('✅ Initialized UserData - token: ${_userData?.token != null && _userData!.token.length > 20 ? "${_userData!.token.substring(0, 20)}..." : _userData?.token ?? "null"}, userId: ${_userData?.userId ?? "null"}, name: ${_userData?.name ?? "null"}');
  }

  static String? _activeRoomCode;

  /// Get currently active room code for collaborative shopping
  static String? get activeRoomCode => _activeRoomCode;

  /// Set the active room code for collaborative shopping
  static void setActiveRoomCode(String? code) {
    _activeRoomCode = code;
    debugPrint('🏠 Active Room Code set to: $code');
  }
  ...
  static Future<void> setUserData(UserDataModel userData) async {
    ...
    _userData = userData;
    debugPrint('✅ Set UserData - token: ${userData.token.substring(0, userData.token.length > 20 ? 20 : userData.token.length)}..., userId: ${userData.userId}, name: ${userData.name}, mobile: ${userData.mobile}');
  }
